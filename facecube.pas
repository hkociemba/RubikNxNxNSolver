...
uses
  Classes, SysUtils, Graphics, cubedefs, phase1_tables;
...
type
  ...
  public
    ...
    procedure createUDXCrossMoveTable(x: integer);
...
var
  ...
  FBXCrossMoveX: array of array of UInt32;

implementation

uses Windows, main, Forms;
...
var
  pm: moves;
begin
  if currMove = yB3 then // fertig
    Exit(NoMove);
  if idx = 0 then
    Exit(Succ(currMove))
  else
  begin
    pm := fxymoves[idx - 1]; // Vorgängerzug
    while True do
    begin
...
// die move-tables sollten eigentlich für alle 1<=x<y<=(size-1)div 2 gleich sein
procedure faceletCube.createUDXCrossMoveTable(x: integer);
var
  i, k: integer;
  a: Axis;
  fs: TFileStream;
const
  ...
  fF = 'UDXCrossMoveF';
begin
  SetLength(UDXCrossMoveX, B_24_8, 3 * 6);
  SetLength(UDXCrossMoveF, B_24_8, 3 * 6);
  if FileExists(fX) and FileExists(fF) then
  begin
    fs := TFileStream.Create(fX, fmOpenRead);
    for i := Low(UDXCrossMoveX) to High(UDXCrossMoveX) do
      fs.ReadBuffer(UDXCrossMoveX[i][0], SizeOf(UInt32) * 3 * 6);
    fs.Free;
    fs := TFileStream.Create(fF, fmOpenRead);
    for i := Low(UDXCrossMoveF) to High(UDXCrossMoveF) do
      fs.ReadBuffer(UDXCrossMoveF[i][0], SizeOf(UInt32) * 3 * 6);
    fs.Free;
  end
  else
  begin
    ...
    begin
      InvPhase1CenterCoord(i, x, x);
      for a := U to B do
      begin
        for k := 0 to 3 do
        begin
          move(a, x + 1); // move hat 1 offset, beginnt bei 1
          if k <> 3 then
            UDXCrossMoveX[i, 3 * Ord(a) + k] := Phase1CenterCoord(x, x);
        end;
      end;
    end;
    fs := TFileStream.Create(fX, fmCreate);
    for i := Low(UDXCrossMoveX) to High(UDXCrossMoveX) do
      fs.WriteBuffer(UDXCrossMoveX[i][0], SizeOf(UInt32) * 3 * 6);
    fs.Free;
    for i := 0 to B_24_8 - 1 do
    begin
      InvPhase1CenterCoord(i, x, x);
      ...
      begin
        for k := 0 to 3 do
        begin
          move(a, 1); // move hat 1 offset, beginnt bei 1
          if k <> 3 then
            UDXCrossMoveF[i, 3 * Ord(a) + k] := Phase1CenterCoord(x, x);
        end;
      end;
    end;
    fs := TFileStream.Create(fF, fmCreate);
    for i := Low(UDXCrossMoveF) to High(UDXCrossMoveF) do
      fs.WriteBuffer(UDXCrossMoveF[i][0], SizeOf(UInt32) * 3 * 6);
    fs.Free;
  end;
end;
